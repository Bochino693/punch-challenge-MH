...
##      `embed_pck` faz o `.exe` parecer autossuficiente. Copiar só o
##      executável é a causa número um de "funciona no meu PC";
...
func _trabalhar() -> void:
	...
	if not _tem_extensao:
		_dizer("FALTA A EXTENSÃO DA CÂMERA AO LADO DO JOGO.")
		_dizer("Copie a PASTA INTEIRA da exportação, não só o .exe.")
		_terminar()
		return
