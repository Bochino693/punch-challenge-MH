...
## O ÍCONE DO JOGO: luva vermelha sobre placa creme.
##
##     godot --headless --path . --script tools/gerar_icone.gd
##
## Era um script em Python que montava o PNG byte a byte, com `zlib` e
## `struct` — trezentas linhas para fazer o que o motor já faz. Aqui é a
## mesma imagem desenhada com `Image`, que sabe gravar PNG sozinha.
##
## POR QUE UM DESENHO E NÃO UM ARQUIVO PRONTO: o mesmo motivo do banco de
## áudio. Um ícone baixado é licença de terceiro multiplicada por cada
## gabinete; desenhado aqui, é do projeto, e mudar a cor da placa é mudar
## uma linha.
