...
func gerar() -> void:
	...
	var par := publico()
	d.salvar(pasta, "arena_publico", par[0], par[1], false, 0.11)
	# Reação exclusiva do golpe abaixo de 6.000; não compartilha o áudio
	# de comemoração para não premiar um golpe fraco.
	par = torcida_desdenho()
	d.salvar(pasta, "torcida_desdenho", par[0], par[1], false, 0.105)
	for festa in [
		["torcida_recorde", 5.4, 1.00],
		["torcida_podio", 4.4, 0.82],
		["torcida_top10", 3.5, 0.62],
		["torcida_top20", 2.7, 0.44],
	]:
		var intensidade := float(festa[2])
		par = torcida_estadio(float(festa[1]), intensidade)
		d.salvar(pasta, str(festa[0]), par[0], par[1], false,
			0.13 if intensidade >= 0.8 else 0.11)
